...
def print_and_save(results, sequences, log_dir, epoch=None):
    ...
    def capture_print(*args, **kwargs):
        ...
        print_content["messages"].append(message)
        print(*args, **kwargs)  

    avg_seq_len = np.mean(results)
    chain_sr = {i + 1: sr for i, sr in enumerate(count_success(results))}
    
    capture_print(f"Average successful sequence length: {avg_seq_len}")
    capture_print("Success rates for i instructions in a row:")
    for i, sr in chain_sr.items():
        capture_print(f"{i}: {sr * 100:.1f}%")
    ...
    tasks_sorted = sorted(
        total.keys(),
        key=lambda task: cnt_success[task] / total[task] if total[task] > 0 else 0,
        reverse=True
    )

    for task in tasks_sorted:
        sr_percent = cnt_success[task] / total[task] * 100 if total[task] > 0 else 0
        capture_print(f"{task}: {cnt_success[task]} / {total[task]} | SR: {sr_percent:.1f}%")

    with open(log_dir, "w") as file:
        json.dump(print_content, file, indent=2, ensure_ascii=False)
