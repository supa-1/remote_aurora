...
torchrun --nproc-per-node=8 --nnodes 1 --node_rank 0 \
    ...
    --data_path /dataset/task_ABCD_D/training_r5.json \
    /dataset/pretrain/bridge_pretrain.json\
    /dataset/pretrain/libero_pretrain.json\
    --data_proportions 1   0.0625 1\
    --image_folder /dataset/task_ABCD_D/vla_processed_r5 \
    --target_image_folder /dataset/task_ABCD_D/vla_processed_r5 \
