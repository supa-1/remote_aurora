...
class ActionTokenizer:
    ...
    def vocab_size(self) -> int:
        return self.n_bins
    # use_norm_bins is for the bins with more density around 0, which is more suitable for the normalized action space.
...
def encode_actions(sentence, action_tokenizer, statistics=None):
    if statistics is not None:
        ...
        actions = actions * 2 - 1
        real_action_token, actions_lang = action_tokenizer(actions) # id , true action value
    else:
...
def encode_robot_obs(sentence, action_tokenizer, statistics=None):
    ...
    robot_obs = (np.array(robot_obs) - robot_obs_mean) / (robot_obs_std + 1e-5) # avoid division by zero
