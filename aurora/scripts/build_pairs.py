...
from auroraig.data.pair_builder import build_contrastive_pairs
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="构建 AuroraIG 对比学习样本")
    p.add_argument(
        "--reconvla_json",
        default="/home/supa1/myreconvla/AuroraIG/data/processed_json/calvin_debug_dataset/training_r5.json",
        help="Reconvla 训练 JSON 路径（默认使用 AuroraIG/data/calvin_debug training_r5.json）",
    )
    p.add_argument(
        "--output_jsonl",
        default="/home/supa1/myreconvla/AuroraIG/data/contrastive_pairs/calvin_debug_training_llm.jsonl",
        help="输出对比学习 JSONL 路径",
    )
    ...
    p.add_argument(
        ...
        action="store_true",
        help="当 LLM 无返回时，不回退规则改写（纯 LLM 构建）",
    )
    p.add_argument(
        "--min_pairs",
        type=int,
        default=1,
        help="最少生成 pair 数；低于该值直接报错",
    )
...
def main() -> None:
    ...
    print(f"done: wrote {total} pairs to {args.output_jsonl}")
